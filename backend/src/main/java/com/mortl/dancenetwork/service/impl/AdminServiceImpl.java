...
public class AdminServiceImpl implements IAdminService {
  ...
  private Map<YearMonth, Long> getTicketsPerMonth(LocalDateTime createdAt, long eventId) {
    ...
    List<Object[]> ticketCounts = ticketRepository.countTicketsByEventIdAndMonth(eventId, createdAt, now);
  ...
  private Map<Role, Long> getTicketsByRole(long eventId){
    List<Object[]> result = ticketRepository.countTicketsByRole(eventId);
    Map<Role, Long> roleCountMap = new HashMap<>();

    for (Object[] record : result) {
      Role role = (Role) record[0];
      long count = (long) record[1];
  ...
  private List<Pair<String, Pair<Long, Long>>> getTicketsByTicketType(long eventId) {
    ...
    for (Object[] record : ticketCounts) {
      ...
      TicketType foundTicketType = allTicketTypes.stream().filter(ticketType -> ticketType.getId() == ticketTypeId).findAny()
          .orElseThrow(() -> new NoSuchElementException("Found ticket with ticketTypeId " + ticketTypeId + " but no such ticketType"));
    ...
    allTicketTypes.stream().forEach(ticketType -> result.add(Pair.of(ticketType.getName(), Pair.of(0L, ticketType.getContingent()))));
